...
export function HealthDataManagement() {
  ...
  // Mock health data
  const [birthDate, setBirthDate] = useState<Date | undefined>(new Date(1990, 4, 15));
  ...
  const handleSaveSection = async (section: SectionType) => {
    ...
    try {
      ...
      if (section === "basic") {
        if (birthDate) {
          payload.age = differenceInYears(new Date(), birthDate);
        }
        payload.gender = formData.gender === "male" ? "남성" : "여성";
        if (formData.height) payload.height = parseFloat(formData.height);
        if (formData.weight) payload.weight = parseFloat(formData.weight);
        if (!waistUnknown && formData.waist) payload.waist = parseFloat(formData.waist);
      }

      if (section === "lifestyle") {
        payload.drinking = formData.drinksAlcohol === "yes" ? "음주함" : "음주안함";
        if (formData.drinksAlcohol === "yes") {
          if (formData.drinksPerSession) payload.drink_amount = parseFloat(formData.drinksPerSession);
          if (formData.alcoholType) {
            const typeMap: Record<string, string> = { soju: "소주", beer: "맥주", other: "기타" };
            payload.drink_type = typeMap[formData.alcoholType] ?? formData.alcoholType;
          }
          if (formData.drinkingFrequency) payload.weekly_drink_freq = parseFloat(formData.drinkingFrequency);
        }
        payload.exercise = formData.exercises === "yes" ? "운동함" : "운동안함";
        if (formData.exercises === "yes" && formData.exerciseFrequency) {
          payload.weekly_exercise_count = parseInt(formData.exerciseFrequency);
        }
        ...
        payload.smoking = smokingMap[formData.smokes] ?? "흡연경험없음";
        payload.current_smoking = formData.smokes === "yes" ? "매일" : "안함";
        if (formData.sleepHours) payload.sleep_hours = parseFloat(formData.sleepHours);
      }

      if (section === "diet") {
        payload.diet_questions = [
          formData.dietQ1, formData.dietQ2, formData.dietQ3, formData.dietQ4,
          formData.dietQ5, formData.dietQ6, formData.dietQ7,
        ].map(Number);
      }

      if (section === "diagnosis") {
        payload.diabetes = formData.diabetes === "yes" ? "예" : "아니오";
        payload.hypertension = formData.hypertension === "yes" ? "예" : "아니오";
        payload.sleep_disorder = formData.sleepDisorder === "yes" ? "예" : "아니오";
      }

      await surveyService.update(payload);
          ...
          {expandedSection !== "basic" ? (
            ...
            <form className="space-y-6">
              ...
              <div className="flex gap-3 pt-4">
                ...
                <Button
                  type="button"
                  disabled={isLoading} onClick={() => handleSaveSection("basic")}
          ...
          {expandedSection !== "lifestyle" ? (
            ...
            <form className="space-y-6">
              ...
              <div className="flex gap-3 pt-4">
                ...
                <Button
                  type="button"
                  disabled={isLoading} onClick={() => handleSaveSection("lifestyle")}
                  className="flex-1 bg-gradient-to-r from-emerald-500 to-teal-600 hover:from-emerald-600 hover:to-teal-700"
                >
                  저장
                </Button>
          ...
          {expandedSection !== "diet" ? (
            ...
            <form className="space-y-6">
              ...
              <div className="flex gap-3 pt-4">
                ...
                <Button
                  type="button"
                  disabled={isLoading} onClick={() => handleSaveSection("diet")}
                  className="flex-1 bg-gradient-to-r from-emerald-500 to-teal-600 hover:from-emerald-600 hover:to-teal-700"
                >
                  저장
                </Button>
          ...
          {expandedSection !== "diagnosis" ? (
            ...
            <form className="space-y-6">
              ...
              <div className="flex gap-3 pt-4">
                ...
                <Button
                  type="button"
                  disabled={isLoading} onClick={() => handleSaveSection("diagnosis")}
                  className="flex-1 bg-gradient-to-r from-emerald-500 to-teal-600 hover:from-emerald-600 hover:to-teal-700"
                >
                  저장
                </Button>
