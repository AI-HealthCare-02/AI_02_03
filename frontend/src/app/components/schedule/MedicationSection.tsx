...
export function MedicationSection({ medications, onRefresh }: Props) {
  ...
  const handleFrequencyChange = (frequency: string) => {
    setNewMedication((prev) => ({
      ...prev,
      frequency,
      times: getRecommendedTimes(frequency, prev.timing),
    }));
  };
  ...
  const handleTimingChange = (timing: string) => {
    setNewMedication((prev) => ({
      ...prev,
      timing,
      times: getRecommendedTimes(prev.frequency, timing),
    }));
  };
  ...
  const confirmDelete = async () => {
    ...
    await api.delete(`/api/v1/medications/${medicationToDelete.id}`);
  ...
  return (
    <>
      <Card className="border-2 border-gray-200">
        ...
        {isOpen && (
          <CardContent className="space-y-4 border-t">
            {isAdding && (
              <div className="p-4 bg-purple-50/50 rounded-lg border border-gray-200">
                ...
                <div className="space-y-4">
                  <div className="space-y-2 relative">
                    ...
                    {showSuggestions && medicationNameInput && filteredMedications.length > 0 && (
                      <div className="absolute z-10 w-full bg-white border-2 border-gray-200 rounded-lg mt-1 shadow-lg max-h-40 overflow-y-auto">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="space-y-2">
                      {newMedication.times.length > 0 ? (
                        ...
                        <p className="text-sm text-gray-500 p-3 bg-white border-2 border-gray-200 rounded-lg">
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>{medicationToDelete?.name} 복용을 중단할까요?</AlertDialogTitle>
            <AlertDialogDescription>
              복용 중단 이후에는 현재 복용 중 목록에서 제외됩니다.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
