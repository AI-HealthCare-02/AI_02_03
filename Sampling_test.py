...
# 건강 로그 저장
res = requests.post(f"{BASE_URL}/health-logs", headers=headers, json={
    "log_date": "2025-06-01",
    "weight": 70.5,
...
# 복약 생성
res = requests.post(f"{BASE_URL}/medications", headers=headers, json={
    "name": "혈압약", "dosage": "1정", "times": ["08:00"]
})
...
# 아주 긴 닉네임
res = requests.patch(f"{BASE_URL}/users/me", headers=headers, json={
    "nickname": "닉" * 100
})
...
# 이상한 성별 값으로 설문
res = requests.post(f"{BASE_URL}/surveys", headers=headers, json={
    "age": 30, "gender": "alien",
    ...
    "sleep_hours": 7.0, "sleep_disorder": "없음",
