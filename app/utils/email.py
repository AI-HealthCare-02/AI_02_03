...
async def send_email_verification_code(to: str, code: str) -> None:
    if not config.SMTP_USER or not config.SMTP_PASSWORD:
        logger.warning("SMTP 설정이 없어 이메일 발송을 건너뜁니다. to=%s", to)
        return

    msg = MIMEMultipart("alternative")
    msg["Subject"] = "[간건강 케어] 이메일 변경 인증 코드"
    msg["From"] = config.SMTP_USER
    msg["To"] = to
    msg.attach(
        MIMEText(
            f"""
    <div style="font-family: sans-serif; max-width: 480px; margin: 0 auto; padding: 32px;">
      <h2 style="color: #059669;">간건강 케어 이메일 변경 인증</h2>
      <p>아래 인증 코드를 입력해 이메일 변경을 완료해주세요.</p>
      <div style="background: #f0fdf4; border: 1px solid #a7f3d0; border-radius: 8px;
                  padding: 16px; text-align: center; margin: 24px 0;">
        <p style="margin: 0; font-size: 13px; color: #6b7280;">인증 코드</p>
        <p style="margin: 8px 0 0; font-size: 32px; font-weight: bold;
                  color: #065f46; letter-spacing: 8px;">{code}</p>
      </div>
      <p style="color: #ef4444; font-size: 13px;">⚠ 이 코드는 <strong>10분 후 만료</strong>됩니다.</p>
